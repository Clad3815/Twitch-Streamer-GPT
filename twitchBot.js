...
const promptsConfig = JSON.parse(
  fs.readFileSync("./prompts/prompts.json", "utf-8")
);
...
if (process.env.ENABLE_TWITCH_ONRESUB === "1") {
  tmiClient.on(
    ...
    (channel, username, streakMonths, message, userstate, methods) => {
      ...
      handleTwitchEvent(handleOnResub, {
...
if (process.env.ENABLE_TWITCH_ONCOMMUNITYSUB === "1") {
  tmiClient.on(
    ...
    (channel, username, numbOfSubs, methods, userstate) => {
      let senderCount = ~~userstate["msg-param-sender-count"];
      handleTwitchEvent(handleOnCommunitySub, {
        broadcasterName: channel,
        gifterName: username,
        giftSubCount: numbOfSubs,
        totalGiftSubCount: senderCount,
      });
    }
  );
}
...
if (process.env.ENABLE_TWITCH_ONPRIMEPAIDUPGRADE === "1") {
  bot.onPrimePaidUpgrade((data) =>
    handleTwitchEvent(handleOnPrimePaidUpgrade, data)
  );
}

if (process.env.ENABLE_TWITCH_ONGIFTPAIDUPGRADE === "1") {
  bot.onGiftPaidUpgrade((data) =>
    handleTwitchEvent(handleOnGiftPaidUpgrade, data)
  );
}
...
function readRandomWaitMP3() {
  ...
  console.log(
    "Playing wait mp3: " + path.join(__dirname, "wait_mp3", randomMP3)
  );
...
async function handleOnSub({ broadcasterName, userName }) {
  ...
  const prompt = promptsConfig.onSub
    .replace("{userName}", userName)
    .replace("{broadcasterName}", broadcasterName);
...
async function handleOnResub({ broadcasterName, userName, months }) {
  ...
  const prompt = promptsConfig.onResub
    .replace("{userName}", userName)
    .replace("{broadcasterName}", broadcasterName)
...
async function handleOnSubGift({
  broadcasterName,
  gifterName,
  recipient,
  totalGiftSubCount,
  ...
  const prompt = promptsConfig.onSubGift
    .replace("{userName}", recipient)
    .replace("{gifterName}", gifterName)
    .replace("{broadcasterName}", broadcasterName);
  openaiLib.answerToMessage(userData, prompt).then((message) => {
    bot.say(channelName, message);
  });
...
async function handleOnCommunitySub({
  broadcasterName,
  gifterName,
  giftSubCount,
  totalGiftSubCount,
  ...
  const prompt = promptsConfig.onCommunitySub
    .replace("{gifterName}", gifterName)
    .replace("{broadcasterName}", broadcasterName)
    .replace("{giftSubCount}", giftSubCount)
    .replace("{totalGiftSubCount}", totalGiftSubCount);
  openaiLib.answerToMessage(userData, prompt).then((message) => {
    bot.say(channelName, message);
  });
}
...
async function handleOnPrimePaidUpgrade({ broadcasterName, userName }) {
  ...
  const prompt = promptsConfig.onPrimePaidUpgrade
    .replace("{userName}", userName)
    .replace("{broadcasterName}", broadcasterName);
  openaiLib.answerToMessage(userData, prompt).then((message) => {
    bot.say(channelName, message);
...
async function handleOnGiftPaidUpgrade({
  broadcasterName,
  userName,
  gifterDisplayName,
}) {
  ...
  const prompt = promptsConfig.onGiftPaidUpgrade
    .replace("{userName}", userName)
    .replace("{gifterDisplayName}", gifterDisplayName)
    .replace("{broadcasterName}", broadcasterName);
  openaiLib.answerToMessage(userData, prompt).then((message) => {
    bot.say(channelName, message);
  });
...
async function main() {
      ...
      const redemptionAction = async () => {
        ...
        if (redemptionTrigger == message.rewardTitle) {
          ...
          if (!(await openaiLib.analyseMessage(message.message))) {
            bot.say(
              channelName,
              promptsConfig.warningMessage.replace(
                "{userName}",
          ...
          const enableGoogleTTS =
            process.env.READ_CHANNEL_POINT_REDEMPTIONS === "1";
          if (enableGoogleTTS) {
            console.log("Generating TTS of the message");
            const ttsPrompt = promptsConfig.ttsMessage
              .replace("{userDisplayName}", message.userDisplayName)
          ...
          const answerMessage = await openaiLib.answerToMessage(
            userData,
            message.message
          );
          bot.say(channelName, answerMessage);
		  
		fs.writeFile('output.txt', answerMessage, (err) => {
		if (err) throw err;
		});
        }
      };
  ...
  if (process.env.ENABLE_TWITCH_ONBITS === "1") {
    pubSubClient.onBits(user.id, async (message) => {
      const bitsAction = async () => {
        const minBits = process.env.TWITCH_MIN_BITS
          ? parseInt(process.env.TWITCH_MIN_BITS)
          : 0;
  ...
  setInterval(async () => {
    ...
    streamInfos.followers = userFollowers.total;
    streamInfos.description = user.description;
    const stream = await broadcasterApiClient.streams.getStreamByUserId(
      user.id
    );
    if (stream) {
      streamInfos.gameName = stream.gameName;
...
async function getViewerInfos(viewer_name) {
  ...
  if (user.id === broadcaster.id) {
    ...
    try {
      ...
      const isVip = await broadcasterApiClient.channels.checkVipForUser(
        channel,
        user
      );
      const isMod = await broadcasterApiClient.moderation.checkUserMod(
        channel,
        user
      );
      return {
